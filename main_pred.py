...
head_scaling = 0.5                    # 0.15*1.1
eye_scaling = head_scaling*10         # 3*1.1
...
cap = cv2.VideoCapture('E:\\projects\\gaze-tracking-project\\vids\\test_vid.mp4')
filename = 'test_vid1.mp4'
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter(filename, fourcc, 20, (1280,  720))
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        # print("scaling head: ",scaling['head'])
        # print("scaling eye: ",scaling['eye'])
        ...
        x2 = p1[0]+int(head_vector[0]*scaling['head']+eye_vector[0]*scaling['eye'])
        y2 = p1[1]+int(head_vector[1]*scaling['head']+eye_vector[1]*scaling['eye'])
        if x2>image.shape[1]:
        ...
        cv2.putText(image_headEye, f'DEPTH: {int(depth/10)}', (20,300), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,255,0), 2)
